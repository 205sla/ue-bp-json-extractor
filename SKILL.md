...
2. Confirm the asset path and Unreal engine version, for example `VER_UE5_5`.
...
11. When UAssetGUI fails, inspect the `string_inventory_json` fallback if present. It is a lossy ASCII identifier inventory from the binary asset, useful for names/references when full parsing fails. Disable it only for very large batches with `-NoStringFallback`.
12. Use `scripts/summarize_uasset_json.py` directly when raw UAssetGUI JSON already exists. This works on any platform with Python 3.10+ and is the primary path for non-Windows hosts.
13. For batch folders, inspect the generated `manifest.json` or `manifest.jsonl` first, then open only the summary JSON files needed for the task. The wrapper updates the manifest after each asset, so interrupted batches can still leave partial progress.
14. Include raw export blobs only when explicitly needed by passing `-IncludeRaw`; otherwise keep raw bytes out of the AI summary.
